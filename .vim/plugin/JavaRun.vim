...
function! ProgRun(...)
	...
	if ext == "java" && getftime(expand("%:r") . ".class") < getftime(expand("%"))
		make
		let e = v:shell_error
    elseif ext == "scala" && getftime(expand("%:r") . ".class") < getftime(expand("%"))
		make
		let e = v:shell_error
	elseif ext == "c"
		make
	...
	if e == 0
		if exists("g:runprogstring")
			...
			if ext == "java"
				execute "!java " . expand("%:r") . " " . arg
            elseif ext == "scala"
				execute "!scala " . expand("%:r") . " " . arg
			elseif ext == "py"
				execute "!python " . expand("%") . " " . arg
" 			elseif ext == "py"
" 				execute "!python2 " . expand("%") . " " . arg
			elseif ext == "lua"
				execute "!lua " . expand("%") . " " . arg
			elseif ext == "go"
				execute "!go run " . expand("%") . " " . arg
			elseif ext == "pl"
			...
			elseif ext == "f" || ext == "f90"
				execute "!./" . expand("%:r") . " " . arg
			elseif ext == "sh"
				execute "!sh" expand("%") . " " . arg
			elseif ext == "tex"
				execute "!epdfview " expand("%:r") . ".pdf" . " " . arg
			endif
			cd -
		endif
	endif
